...
public class RedBlackTree<K extends Comparable<? super K>, V> {
  ...
  private void insertCaseFour(Node<K, V> current) {
    Node<K, V> oldParent = current.getParent();
    if (current.getParent() != null && current.getParent().isLeftChild()
        && current.isRightChild()) {
      oldParent.rotateLeft();
      if (oldParent == root) {
        root = current;
      }
      insertCaseFive(oldParent);
    }
    if (current.getParent() != null && current.getParent().isRightChild()
        && current.isLeftChild()) {
      oldParent.rotateRight();
      if (oldParent == root) {
        root = current;
      }
      insertCaseFive(oldParent);
  ...
  private void insertCaseFive(Node<K, V> current) {
    if (current.isLeftChild()) {
      current.getParent().setBlack();
      Node<K, V> grandpa = current.getGrandparent();
      assert grandpa != null;
      grandpa.setRed();
      grandpa.rotateRight();
      if (grandpa == root) {
